    ...
    function formatAppointmentDateTime(dateStr, timeStr) {
      try {
        ...
        // Format: Monday, December 12, 2025 at 12:11 PM
    ...
    app.delete('/api/complete-appointment/:id', (req, res) => {
      try {
        ...
        const appointmentStmt = db.prepare("SELECT id, user_id, date, time, service, stylist FROM appointments WHERE id = ?");
        ...
        if (result.changes > 0) {
          // Create notification with appointment details
          const formattedDateTime = formatAppointmentDateTime(appointment.date, appointment.time);
          const notificationMessage = `Your appointment has been completed. ${formattedDateTime}`;
          const notifStmt = db.prepare(`
            INSERT INTO notifications (user_id, message, type, appointment_id, appointment_date, appointment_time)
            VALUES (?, ?, ?, ?, ?, ?)
          `);
          notifStmt.run(appointment.user_id, notificationMessage, 'success', appointmentId, appointment.date, appointment.time);
    ...
    app.post('/api/send-notification', (req, res) => {
      try {
        const { user_id, message, type, appointment_id, appointment_date, appointment_time } = req.body;

        const notifStmt = db.prepare(`
          INSERT INTO notifications (user_id, message, type, appointment_id, appointment_date, appointment_time)
          VALUES (?, ?, ?, ?, ?, ?)
        `);
        const info = notifStmt.run(user_id, message, type || 'success', appointment_id || null, appointment_date || null, appointment_time || null);
    ...
    app.get('/api/notificationss/:userId', (req, res) => {
      try {
        ...
        const stmt = db.prepare(`
          SELECT id, user_id, message, created_at, appointment_id, appointment_date, appointment_time
          FROM notifications
    ...
    app.patch('/api/update-appointment-status/:id', (req, res) => {
      try {
        ...
        const result = updateStmt.run(status, appointmentId);

        if (result.changes > 0) {
          // Create notification with appointment details
          const formattedDateTime = formatAppointmentDateTime(appointment.date, appointment.time);
          const notificationMessage = `Your appointment has been ${status}. ${formattedDateTime}`;
          const notifStmt = db.prepare(`
            INSERT INTO notifications (user_id, message, type, appointment_id, appointment_date, appointment_time)
            VALUES (?, ?, ?, ?, ?, ?)
          `);
          notifStmt.run(appointment.user_id, notificationMessage, status === 'Approved' ? 'success' : 'info', appointmentId, appointment.date, appointment.time);
